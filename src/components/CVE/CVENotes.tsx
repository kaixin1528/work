...
import {
  ...
  EditCVENote,
  DeleteCVENote,
} from "src/services/general/cve";

const CVENotes = ({ cveID }: { cveID: string }) => {
  const jwt = decodeJWT();
  ...
  const { data: notes } = GetCVENotes(env, cveID, show);
  const addNote = AddCVENote(env, cveID);
  const updateNote = EditCVENote(env, cveID);
  const deleteNote = DeleteCVENote(env, cveID);
  const { data: allUsers } = GetAllUsers(customerID, !show);
  ...
  const handleAddNote = () => {
    ...
    addNote.mutate({
      note: {
        body: newNote,
        tagged_users: getAllEmails(newNote),
      },
    });
    setMention(false);
  };
  const handleEditNote = (note: NoteType) => {
    ...
    updateNote.mutate({
      nodeID: note.id,
      note: {
        body: editCurrentNote,
        tagged_users: getAllEmails(editCurrentNote),
      },
    });
    setMention(false);
  };
  ...
  return (
    <Popover className="relative">
      <Popover.Button
        className="flex items-center justify-center p-2 dark:text-black dark:bg-checkbox dark:hover:bg-checkbox/60 duration-100 focus:outline-none rounded-full"
      ...
      >
        <Popover.Panel className="pointer-events-auto absolute top-10 left-0 flex flex-col flex-grow m-4 md:w-[20rem] lg:w-[30rem] h-[30rem] break-words text-xs dark:bg-card z-10">
          <h4 className="p-2 text-base text-center bg-gradient-to-b dark:from-filter dark:to-filter/30">
            Notes
          </h4>
          {/* add new note */}
