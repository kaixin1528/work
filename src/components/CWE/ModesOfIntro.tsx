...
const ModesOfIntro = ({ selectedCWE }: { selectedCWE: string }) => {
  const { data: cweDetail } = GetCWEDetail(selectedCWE);

  const keys =
    cweDetail?.data.modes_of_introduction?.length > 0
      ? Object.keys(cweDetail?.data.modes_of_introduction[0])
      : [];
            ...
            {cweDetail ? (
              cweDetail?.data.modes_of_introduction?.length > 0 ? (
                <TableLayout>
                  ...
                  <tbody>
                    {cweDetail?.data.modes_of_introduction?.map(
