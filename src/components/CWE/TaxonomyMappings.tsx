...
const TaxonomyMappings = ({ selectedCWE }: { selectedCWE: string }) => {
  const { data: cweDetail } = GetCWEDetail(selectedCWE);

  const keys =
    cweDetail?.data.taxonomy_mappings?.length > 0
      ? Object.keys(cweDetail?.data.taxonomy_mappings[0])
      : [];
            ...
            {cweDetail ? (
              cweDetail?.data.taxonomy_mappings?.length > 0 ? (
                <TableLayout>
                  ...
                  <tbody>
                    {cweDetail?.data.taxonomy_mappings?.map(
...
export default TaxonomyMappings;
