...
const CommonSequences = ({ selectedCWE }: { selectedCWE: string }) => {
  const { data: cweDetail } = GetCWEDetail(selectedCWE);

  const keys =
    cweDetail?.data.common_consequences?.length > 0
      ? Object.keys(cweDetail?.data.common_consequences[0])
      : [];
            ...
            {cweDetail ? (
              cweDetail?.data.common_consequences?.length > 0 ? (
                <TableLayout>
                  ...
                  <tbody>
                    {cweDetail?.data.common_consequences?.map(
...
export default CommonSequences;
