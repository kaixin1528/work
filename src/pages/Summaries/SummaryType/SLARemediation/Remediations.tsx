...
import { GetSLARemediations } from "src/services/summaries/sla-remediation";
...
const Remediations = ({
  ...
  const { data: remediations } = GetSLARemediations(
    selectedSource,
    selectedVersion,
    selectedIntegrationType,
    selectedSourceAccountID,
  ...
  const totalCount = remediations?.pager.total_results || 0;
  const totalPages = remediations?.pager.num_pages;
  const beginning =
    pageNumber === 1 ? 1 : pageSize * ((pageNumber || 1) - 1) + 1;
  const end = pageNumber === totalPages ? totalCount : beginning + pageSize - 1;
  const sortedRemediations = sortRows(remediations?.data, sort);
          ...
          {sortedRemediations?.length > 0 ? (
            <TableLayout height="max-h-[20rem]">
              <thead className="sticky -top-1.5 z-10 bg-gradient-to-b dark:from-expand dark:to-table-header">
                <tr>
                  {remediations.metadata.headers.map((col: ListHeader) => {
                    return (
                      <th
                        scope="col"
