  ...
  return (
    <section className="flex flex-col flex-grow gap-5">
      <a
        ...
        className="text-base dark:hover:text-checkbox/60 duration-100"
      >
            ...
            {reviewList?.data.map((review: any, index: number) => {
              return (
                <li
                  ...
                  onClick={() => {
                    sessionStorage.GRCCategory = "privacy review";
                    navigate(
                      `/third-party-risk/privacy/details?review_id=${review.review_id}&framework_id=${review.framework_ids[0]}&audit_id=${review.audit_id}`
                    );
                  }}
                >
                  {review.status === "READY" ? (
