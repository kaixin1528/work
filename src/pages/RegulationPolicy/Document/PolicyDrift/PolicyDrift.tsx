...
const PolicyDrift = () => {
  ...
  const { data: policyDrift, status: driftStatus } = GetPolicyDrift(
    String(parsed.policy_id)
  );
  const { data: diff, status: diffStatus } = GetPolicyDriftDiff(
    ...
    <PageLayout>
      ...
      >
        <header className="flex items-center gap-5">
          <ReturnPage />
          <h4 className="text-xl">Policy Drift - {parsed.policy_name}</h4>
        </header>
        {driftStatus === "loading" ? (
          <Loader />
        ) : (
