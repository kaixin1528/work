...
const BIADrift = () => {
  ...
  const { data: biaDrift, status: driftStatus } = GetBIADrift(
    String(parsed.bia_id)
  );
  const { data: diff, status: diffStatus } = GetBIADriftDiff(
    ...
    <PageLayout>
      ...
      >
        <header className="flex items-center gap-5">
          <ReturnPage />
          <h4 className="text-xl">Policy Drift - {parsed.bia_name}</h4>
        </header>
        {driftStatus === "loading" ? (
          <Loader />
        ) : (
