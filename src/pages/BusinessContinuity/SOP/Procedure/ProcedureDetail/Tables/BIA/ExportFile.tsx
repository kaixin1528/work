...
const ExportFile = ({
  ...
  const exportBIA = ExportBIA(versionID);
  const { data: exportStatus, refetch } = GetExportBIAStatus(versionID);
  const downloadBIA = DownloadBIA(versionID);
  ...
  return (
    <section className="flex items-center gap-5">
      <button
        ...
        onClick={() => {
          exportBIA.mutate({ selectedColumns: selectedColumns });
        }}
      >
